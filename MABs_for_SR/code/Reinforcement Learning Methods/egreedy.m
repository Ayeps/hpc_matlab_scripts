...
%     end
    ...
    while(iteration < totalIterations+1) 
        ...
        powerMatrix = PowerMatrix(wlansAux);    
        tptExperiencedByWlan(iteration,:) = ...
            compute_throughput_from_sinr(wlansAux, powerMatrix, NOISE_DBM);  % bps
        
        % Update the exploration coefficient according to the inputted mode
        if updateModeEpsilon == UPDATE_MODE_FAST
            epsilon = initialEpsilon / iteration;    
        elseif updateModeEpsilon == UPDATE_MODE_SLOW
