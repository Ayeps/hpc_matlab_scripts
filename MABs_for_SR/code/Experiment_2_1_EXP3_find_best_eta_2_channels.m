...
for r = 1 : totalRepetitions
    ...
    for g = 1 : size(gamma, 2)
        ...
        for e = 1 : size(eta, 2)         
            [throughputEvolutionPerWlan{r}, actionsProbability] = exp3(wlans, gamma(g), eta(e));
...
maximumAchievableThroughput = computeTptAllCombinations(wlans, ...
...
auxArray = zeros(size(gamma, 2), size(eta, 2));
for g = 1 : size(gamma, 2)     
    for e = 1 : size(eta, 2)
...
auxArrayStd = zeros(size(gamma, 2), size(eta, 2));
for g = 1:size(gamma, 2)     
    for e = 1:size(eta, 2)
        for i = 1:totalRepetitions
...
axis([1 20 30 70]);
for g = 1:size(gamma, 2)   
    errorbar(eta, meanAggregateThroughputPerEpsilon(g, :), stdAggregateThroughputPerEpsilon(g, :), '-d');
    hold on
    l = [l ['\gamma = ' num2str(gamma(g))]];
end
plot(eta, optimalAggregateThroughput * ones(1, size(eta, 2)), '--r');
set(gca, 'FontSize', 22)
xlabel('\eta','FontSize', 24)
ylabel('Network Throughput (Mbps)','FontSize',24)
axis([min(eta) max(eta) 0 1.1 * max(optimalAggregateThroughput)])
