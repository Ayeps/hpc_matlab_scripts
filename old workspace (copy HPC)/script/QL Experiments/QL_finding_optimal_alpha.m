...
gamma_ql = .8;  % Discount factor Q-learning
...
for iter = 1:TOTAL_ROUNDS
    ...
    for i = 1:size(alpha,2)
        ...
        tpt_evolution_per_wlan_ql{iter}  = QlearningMethod(wlan, MAX_CONVERGENCE_TIME, MAX_LEARNING_ITERATIONS, ...
                                        gamma_ql, initial_epsilon, final_epsilon, alpha(i), actions_ch, actions_cca, actions_tpc, noise);
...
xticks(1:size(alpha,2))
l = {};
for i=1:size(alpha,2)
    l = [l ['\alpha = ' num2str(alpha(i))]];
end
xticklabels(l)
