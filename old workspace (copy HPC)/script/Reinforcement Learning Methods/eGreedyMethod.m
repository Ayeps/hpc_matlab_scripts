...
function [tpt_experienced_by_WLAN] = eGreedyMethod(wlan, MAX_CONVERGENCE_TIME, MAX_LEARNING_ITERATIONS, ...
                                                    initial_epsilon, updateMode, actions_ch, actions_cca, actions_tpc, noise, printInfo)
% eGreedyMethod - Given an OBSS, applies e-greedy to maximize the experienced throughput
%
%   OUTPUT: 
...
%       * noise - floor noise in dBm
    ...
    reward_per_configuration = zeros(n_WLANs, K); 
    ...
    while(t < MAX_CONVERGENCE_TIME+1) 
        ...
        for i=1:n_WLANs % Iterate sequentially for each agent in the random order                      
            ...
            while(learning_iteration <= MAX_LEARNING_ITERATIONS)
                ...
                selected_arm(order(i)) = selectActionEGreedy(reward_per_configuration(order(i),:), epsilon);
                ...
                for wlan_i = 1:n_WLANs
                    ...
                    reward_per_configuration(wlan_i, selected_arm(wlan_i)) = rw;
                end   
    ...
    if printInfo
        ...
        for i=1:n_WLANs      
            ...
            [~, ix] = max(reward_per_configuration(i, :));
