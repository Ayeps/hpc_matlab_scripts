...
function [tpt_experienced_by_WLAN] = EXP3Method(wlan, MAX_CONVERGENCE_TIME, MAX_LEARNING_ITERATIONS, ...
                                                    eta, actions_ch, actions_cca, actions_tpc, noise)
% EXP3Method Given an OBSS, applies EXP3 (with losses) to maximize the experienced throughput
%
%   OUTPUT: 
...
%                 end
                ...
                for wlan_i = 1:n_WLANs
                    ...
                    loss = tpt_after_action(wlan_i) / upper_bound_tpt_per_wlan(wlan_i);
                    %loss = (upper_bound_tpt_per_wlan(wlan_i) - tpt_after_action(wlan_i)) / upper_bound_tpt_per_wlan(wlan_i);
                    if loss == 0
                        estimated_loss = 1e-6; 
                    else
                        estimated_loss = loss / (p_per_arm(wlan_i, selected_arm(wlan_i)));
                    end
                    ...
                    weights_per_arm(wlan_i, selected_arm(wlan_i)) = ...
                        weights_per_arm(wlan_i, selected_arm(wlan_i)) * ...
                        exp(- eta * estimated_loss);
                    
                    p_per_arm(wlan_i, :) = weights_per_arm(wlan_i, :) / ...
                        sum(weights_per_arm(wlan_i, :));
